...
export class SourceService extends Service<SourceEntity> {
  ...
  override async create(input: CreateSourceDto, manager?: EntityManager): Promise<SourceEntity> {
  ...
  async getUploadUrl(input: GetUploadUrlDto): Promise<FileURLResponseDto> {
  ...
  async downloadUrl(sourceId: string, manager?: EntityManager): Promise<FileURLResponseDto> {
  ...
  async view(sourceId: string, manager?: EntityManager): Promise<FileURLResponseDto> {
    const source = await this.repository(manager).findOneOrFail({ where: { id: sourceId } });
    if (!source) throw new NotFoundException("Source not found");
    return this.storageService.getVisualizationUrl(source.key);
  }

  async retryIndex(sourceId: string, manager?: EntityManager): Promise<SourceEntity> {
    const source = await this.repository(manager).findOneOrFail({ where: { id: sourceId } });
    if (!source) throw new NotFoundException("Source not found");
